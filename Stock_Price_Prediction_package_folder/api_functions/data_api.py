...
def retrieve_data_api(ticker : str):
    '''
...
def retrieve_historical_data_api(data : yf.ticker.Ticker, target : str):
    '''
...
def retrieve_currency_api(data : yf.ticker.Ticker):
    '''
...
def retrieve_short_name_api(data : yf.ticker.Ticker):
    '''
...
def memorize_dates_api(historical_data : pd.DataFrame) :
    ''''
...
def reshape_historical_data_api(historical_data : pd.DataFrame):
    '''
...
def define_train_test_data_api(historical_data : np.ndarray, dates : np.ndarray):
    '''
...
def create_features_target_api(dataset : pd.DataFrame, time_step : int, dates : np.ndarray):
    ''''
